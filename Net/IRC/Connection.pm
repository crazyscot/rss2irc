...
#				    );
    ...
    if ($self->hostname) {
        my $addr = inet_aton($self->hostname) || inet_aton("127.0.0.1");
	$addr = INADDR_ANY
	  unless $addr ne inet_aton("127.0.0.1");
	unless (bind( $sock, sockaddr_in( 0, $addr ) )) {
...
sub parse {
    my ($self) = shift;
    my ($from, $type, $message, @stuff, $itype, $ev, @lines, $line,$lineplus,$dummy);
